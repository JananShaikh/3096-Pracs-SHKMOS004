...
int main(void){
  ...
  //Output decToBcd to putty
  sprintf(buffer, "%d \r\n", decToBcd(12));
  HAL_UART_Transmit(&huart2, buffer, sizeof(buffer), 1000);

  //Output bcdToDec to putty
  sprintf(buffer, "%d \r\n", bcdToDec(18));
  HAL_UART_Transmit(&huart2, buffer, sizeof(buffer), 1000);

  //Set RTC
  setTime(0,0,0,1,1,1,22);
...
void pause_sec(float x)
{
	...
	//YOUR CODE HERE
	//For loop to give delay based on given value of x
...
uint8_t decToBcd(int val)
{
	...
	//YOUR CODE HERE
	//return decimal version of the decimal value converted to bcd
...
int bcdToDec(uint8_t val)
{
	...
	//YOUR CODE HERE
	//return decimal value of the input bcd in decimal form
	int decimal = (val/16*10)+(val%16);
...
void setTime (uint8_t sec, uint8_t min, uint8_t hour, uint8_t dow, uint8_t dom, uint8_t month, uint8_t year)
{
	...
	//YOUR CODE HERE
	//store the give time stamp into set_time
	set_time[0] = sec;
	set_time[1] = min;
	set_time[2] = hour;
	set_time[3] = dow;
...
void getTime (void)
{
	...
	//YOUR CODE HERE
	//retrieve the time stamp returned by the hal function into the time structure
...
int epochFromTime(TIME time){
	...
	//YOUR CODE HERE
	    ...
	    day +=(time.dayofmonth-1); // Number of days passed
	    total +=(time.year-22)*31536000; // number of years passes
	    total +=day*86400; //number of seconds in the days
	    total +=(time.hour+2)*3600; // hours passes in latest not complete day + gmt+2
	    total += time.minutes*60; // minutee passes in latest not complete day
	    total += time.seconds; // seconds passes in latest not complete day



	return EPOCH_2022 + total; // Add to january
}
