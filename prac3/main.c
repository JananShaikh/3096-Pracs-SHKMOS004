...
uint8_t delay = 1; //LED delay (to be multiplied by 500ms)
int timeout = 100; //Timeout value
int timeout_compare = 0; //Next tick to activate IRQ at
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  uint32_t polledADC = pollADC(); //Get ADC value

	  //Test your pollADC function and display via UART
		sprintf(buffer, "%d \r\n", polledADC); //Display ADC value
		HAL_UART_Transmit(&huart2, buffer, sizeof(buffer), 1000);

	  //TASK 3
	  //Test your ADCtoCRR function. Display CRR value via UART
		sprintf(buffer, "%d \r\n", ADCtoCRR(polledADC)); //Calculate and display CRR value
		HAL_UART_Transmit(&huart2, buffer, sizeof(buffer), 1000);

	  //TASK 4
	  //Complete rest of implementation

	  HAL_Delay (delay*500); // wait for 500 ms * delay set
...
void EXTI0_1_IRQHandler(void)
{
	...
	int tick = HAL_GetTick(); //Get current tick time

	if (tick >= timeout_compare){ //Check if tick time is correct to what we set it should be in timeout_compare

		if (delay == 1){ //Swap delay values
			delay = 2;
		}
		else if (delay == 2){
			delay = 1;
	...
	timeout_compare = tick + 100; //Set next tick time
...
uint32_t pollADC(void){
	...
	//ADC polling
	HAL_ADC_Start(&hadc); //Start
	HAL_ADC_PollForConversion(&hadc, timeout); //Poll
	uint32_t val = HAL_ADC_GetValue(&hadc); //Get ADC value
	HAL_ADC_Stop(&hadc); //Stop
...
uint32_t ADCtoCRR(uint32_t adc_val){
	...
	float division = adc_val/4096.0; //Convert ADC value to a ratio
	uint32_t val = (int)(division * 48000); //Use that ratio as a CRR value

	__HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_4, (uint32_t)val); //Set CRR register to CRR value

	return (uint32_t)val; //Return CRR value
}
